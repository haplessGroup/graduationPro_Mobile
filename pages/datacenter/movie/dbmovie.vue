<template>
	<view class="container">
		<view>
			<uni-nav-bar class="mnv" fixed="true">
				<view class="mh">豆瓣电影Top250</view>
		...
		<view class="mtable">
			<uni-table 
				...
				<uni-tr v-for="(item, index) in movieData" :key="index">
		...
		methods: {
			...
			showRateData:function(){
				uni.request({
					...
					method:"GET",
					success: (res) => {
					        this.chartData.categories=res.data[0]
							this.chartData.series[0]["data"]=res.data[1]
					    }
				})
			},
			showMovieData:function(){
				this.loading=true
				uni.request({
					url:"http://127.0.0.1:5000/moviesort",
					method:"GET",
					success:(res)=>{
			...
			// 请求用户评论
			showMovieMessage:function(){
				uni.request({
					url:'http://127.0.0.1:5000/dbmoviemessage',
					method:"GET",
					success: (res) => {
